...
const AddressInput = ({ receiver, setReceiver }: AddressInputProps) => {
  ...
  const getAddonContent = (): React.JSX.Element | null => {
    ...
    if (receiver && !isResolvingInProgress)
      return <Image alt="warning icon" src="/assets/warning.svg" className="icon-wrapper error-icon" width={30} height={30} />
    return null
  }
  ...
  return (
    <Box w="100%">
      <InputGroup>
        ...
        <Input value={receiver} onChange={handleInput} placeholder="Enter Ethereum name or address" name="ethereum" spellCheck={false} />
      </InputGroup>
    </Box>
  )
}
