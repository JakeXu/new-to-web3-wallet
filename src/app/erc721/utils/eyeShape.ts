import { Point, Eye, Points, EyePoints, BothEyes } from '../types'
import { randomFromInterval, cubicBezier } from './base'

function generateEyeParameters(width: number): Eye {
...
export function generateEyePoints(rands: Eye, width = 50): EyePoints {
  let P0_upper: Point = [-width / 2 + (rands.P0_upper_randX * width) / 16, (rands.P0_upper_randY * rands.height_upper) / 16]
  let P3_upper: Point = [width / 2 + (rands.P3_upper_randX * width) / 16, (rands.P3_upper_randY * rands.height_upper) / 16]
  let P0_lower = P0_upper // Starting at the same point as the upper eyelid
  let P3_lower = P3_upper // Ending at the same point as the upper eyelid
  let eye_true_width = P3_upper[0] - P0_upper[0]

  // Upper eyelid control points
  let P1_upper: Point = [P0_upper[0] + rands.offset_upper_left_x, P0_upper[1] + rands.offset_upper_left_y] // First control point
  let P2_upper: Point = [P3_upper[0] - rands.offset_upper_right_x, P3_upper[1] + rands.offset_upper_right_y] // Second control point

  // Lower eyelid control points
  let P1_lower: Point = [P0_lower[0] + rands.offset_lower_left_x, P0_lower[1] - rands.offset_lower_left_y] // First control point
  let P2_lower: Point = [P3_lower[0] - rands.offset_lower_right_x, P3_lower[1] - rands.offset_lower_right_y] // Second control point

  // now we generate the points for the upper eyelid
  let upper_eyelid_points: Points = []
  let upper_eyelid_points_left_control = []
  let upper_eyelid_points_right_control = []
  let upper_eyelid_left_control_point: Point = [
  ...
  let upper_eyelid_right_control_point: Point = [
  ...
  // now we generate the points for the upper eyelid
  let lower_eyelid_points: Points = []
  let lower_eyelid_points_left_control: Points = []
  let lower_eyelid_points_right_control: Points = []
  let lower_eyelid_left_control_point: Point = [
  ...
  let lower_eyelid_right_control_point: Point = [
...
export function generateBothEyes(width = 50): BothEyes {
  ...
  for (let key in rands_right) {
    ...
    const k = key as keyof typeof rands_right
    if (typeof rands_right[k] === 'number') {
      // Add a random value to the number, for example, between -5 and 5
      rands_right[k] += randomFromInterval(-rands_right[k] / 2.0, rands_right[k] / 2.0)
    }
  }
  ...
  for (let key in left_eye) {
    const k = key as keyof typeof left_eye
    if (typeof left_eye[k] === 'object') {
      for (let i = 0; i < left_eye[k].length; i++) {
        left_eye[k][i][0] = -left_eye[k][i][0]
      }
    }
  }
