...
import { randomFromInterval, cubicBezier, getEggShapePoints } from './base'

export function generateMouthShape0(faceHeight: number, faceWidth: number): Points {
  ...
  var mouthRight: Point = [mouthRightX, mouthRightY]
  var mouthLeft: Point = [mouthLeftX, mouthLeftY]

  var controlPoint0: Point = [randomFromInterval(0, mouthRightX), randomFromInterval(mouthLeftY + 5, faceHeight / 1.5)]
  var controlPoint1: Point = [randomFromInterval(mouthLeftX, 0), randomFromInterval(mouthLeftY + 5, faceHeight / 1.5)]
  ...
  if (Math.random() > 0.5) {
    ...
    for (var i = 0; i < 100; i += 1) {
      mouthPoints.push([
        ...
        (mouthPoints[99][1] * (1 - i / 100.0) + (mouthPoints[0][1] * i) / 100.0) * (1 - y_offset_portion) +
...
export function generateMouthShape1(faceHeight: number, faceWidth: number): Points {
  // choose one point on face at bottom side
...
export function generateMouthShape2(faceHeight: number, faceWidth: number): Points {
  ...
  var mouthPoints = getEggShapePoints(
